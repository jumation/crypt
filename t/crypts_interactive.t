...
#                       crypt.slax script in interactive mode, i.e "op crypt"
#                       command. Returned plain-text is compared with predefined
#                       plain-text value. Then the predefined plain-text is
#                       encrypted in Junos device and returned crypt is
#                       compared to pre-defined crypt.
...
for my $t (@tests)
{
	...
	is($decrypt, $plain, "Decrypted '$crypt' returned '$plain'");
	...
	# Newline is added in order to avoid Net::SSH::Expect module "croak
	# (ILLEGAL_ARGUMENT . " missing argument 'string'.") unless ($send);"
	# error when salt is 0.
	$ssh->send($salt . "\n");

	$ssh->read_line();

	my $encrypt = $ssh->read_line();

	substr($encrypt, 4, length($rand), $rand); ## splice in existing rand

	is($encrypt, $crypt,
	"Encrypted '$plain' using salt '$salt' returned $crypt");
