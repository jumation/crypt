...
# Title               : crypts_non-interactive.t
...
# Description         : Script decrypts all the crypts listed after __DATA__
#                       token by logging into Junos device and executing the
#                       "op crypt decrypt <crypt>" command. Returned plain-text
#                       is compared with predefined plain-text value. Then the
#                       predefined plain-text is encrypted in Junos device and
#                       returned crypt is compared to pre-defined crypt.
# Options             : None. Router hostname and username are defined in
#                       variables.
# Notes               : For Net::OpenSSH module, the key-based authentication
#                       is used. Net::OpenSSH takes advantage of SSH multi-
#                       plexing.
...
for my $t (@tests)
{
	...
	my $decrypt_out = $ssh->pipe_out("op crypt decrypt " . $crypt);
	my $decrypt = <$decrypt_out>;
	$decrypt =~ s/\n//;

	is($decrypt, $plain, "\"op crypt decrypt '$crypt'\" returned '$plain'");

	sleep 1;

	my $salt = substr($crypt,3,1);
	my $encrypt_out = $ssh->pipe_out("op crypt encrypt " . $plain .
	" salt " . $salt);
	my $encrypt = <$encrypt_out>;
	$encrypt =~ s/\n//;

	substr($encrypt, 4, length($rand), $rand); ## splice in existing rand

	is($encrypt, $crypt,
	"\"op crypt encrypt '$plain' salt '$salt'\" returned $crypt");
